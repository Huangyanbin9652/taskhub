...
(function(){
  ...
  function showGameRules(){
    ...
    el.innerHTML = `
      ...
      <div class="task-card" style="line-height:1.9;font-size:0.88rem;">
        ...
        <b>出牌</b><br>可出<b>单张</b>、<b>对子</b>、<b>拖拉机</b>（同花色相邻连对，如5566、778899，最多四连对）。主牌中王对、级牌对、主花色连对也构成拖拉机。<br>还可<b>甩牌</b>：首出时同花色多张（单张+对子+拖拉机混合）一起出，但必须确定都是最大牌、别人管不住，否则建议拆开。<br>
  ...
  function confirmPlay(){
    ...
    // 甩牌(sweep)首出：提醒玩家这手并非“全最大牌”，可能被人管住
    const leadCombo = EZ.STATE.trick.length === 0 ? EZ.analyzeCombo(mySelectedCards) : null;
    if(leadCombo && leadCombo.type === 'sweep' && !EZ.canThrowSweep(mySelectedCards, leadCombo.suit, 0)){
      toast('⚠️ 这手甩牌并非全最大牌，可能被管！确定要出吗？');
    }
